...
install: ## install hawk application
	@echo "Installing application ..."
	@-bash -c 'pip install -e .'
	@echo "\nStart service with \`make start\` and stop with \`make stop\`."

.PHONY: develop
develop: ## install hawk application with development libraries
	@echo "Installing development requirements for tests and docs ..."
	@-bash -c 'pip install -e ".[dev]"'

.PHONY: start
start: ## start hawk service as daemon (background process)
	@echo "Starting application ..."
	@-bash -c "$(APP_NAME) start -d"

.PHONY: stop
stop: ## stop hawk service
	@echo "Stopping application ..."
	@-bash -c "$(APP_NAME) stop"

restart: stop start  ## restart hawk service
	@echo "Restarting application ..."

status: ## show status of hawk service
	@echo "Showing status ..."
...
coverage: ## check code coverage quickly with the default Python
	@bash -c 'coverage run --source hawk -m pytest'
