...
from thesis import datasets_and_configurations_loaders, pcmci_tools
...
def main():
    # List all files in the results folder ending with .pkl
    results_files = sorted([file for file in os.listdir(constants.path_results) if file.endswith(".pkl")], key=general_custom_sort_key)
    ...
    for file in results_files:
        ...
        algorithm = parts[0]
        basin = parts[1]
        key = file.split(basin)[1][1:-4]

        if algorithm == "pcmci":
            if basin not in results_pcmci:
                results_pcmci[basin] = {}

            results_pcmci[basin][key] = file_management.load_from_pkl_file(os.path.join(constants.path_results, file))

        elif algorithm == "te":

            if basin not in results_te:
                results_te[basin] = {}

            results_te[basin][key] = file_management.load_from_pkl_file(os.path.join(constants.path_results, file))

    # -------------------- PCMCI --------------------

    for basin_name, basin_results in results_pcmci.items():
        datasets, _, _ = datasets_and_configurations_loaders["pcmci"].get(basin_name)()

        all_basin_variables = set()
        results_table_pcmci = []
        for key, simulation in basin_results.items():
            ...
            if simulation["params"]["lag"] > 0:
                target_file = os.path.join(constants.path_figures, "algorithm_results", basin_name, "pcmci", key + "_timeseries.pdf")
                if not os.path.exists(target_file):
                    fig, ax = plt.subplots()
            ...
            selected_features = pcmci_tools.get_connected_variables(results["graph"], var_names)
        ...
        file_management.save_to_pkl_file(os.path.join(constants.path_table_objects, f"results_table_{basin_name}_pcmci.pkl"), results_table_pcmci)
        ...
        target_file = os.path.join(constants.path_figures, "algorithm_results", basin_name, "pcmci", "feature_presence.pdf")
        os.makedirs(os.path.dirname(target_file), exist_ok=True)
        plt.savefig(target_file, bbox_inches="tight")
        plt.close(fig)
    ...
    for basin_name, basin_results in results_te.items():
        ...
        # Export the file to pkl
        file_management.save_to_pkl_file(os.path.join(constants.path_table_objects, f"results_table_{basin_name}_te.pkl"), results_table_te)
        ...
        target_file = os.path.join(constants.path_figures, "algorithm_results", basin_name, "te", "feature_presence.pdf")
        os.makedirs(os.path.dirname(target_file), exist_ok=True)
        plt.savefig(target_file, bbox_inches="tight")
        plt.close(fig)
...
if __name__ == "__main__":
    ...
    # Optional execution of the notebook

    # import nbformat
    # from nbconvert.preprocessors import ExecutePreprocessor

    # filename = 'droughts_postprocessing_pt2_and_wrapper.ipynb'
    # with open(filename) as ff:
    #     nb_in = nbformat.read(ff, nbformat.NO_CONVERT)

    # ep = ExecutePreprocessor(timeout=600, kernel_name='thesis')
